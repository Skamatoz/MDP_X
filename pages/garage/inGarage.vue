        ...
        <section class="flex flex-col h-screen w-screen ">
            ...
            <section class="overflow-y-auto ">
                ...
                <div  class="flex items-center gap-2 pr-4 ">
                    <form autocomplete="off" @submit.prevent="onSearch"class="flex space-x-2 items-center  py-2 pl-4 w-full sm:w-fit ">
                    ...
                    <button  v-show="filter === true" @click="filterF()" class="text-orange-400 cursor-pointer flex items-center px-4 text-sm bg-gray-100 h-10 w-fit rounded-xl hover:bg-gray-200">
                        Bike
                    </button>
                    <button  v-show="filter === false" @click="filterF()" class="text-orange-400 cursor-pointer flex items-center px-4 text-sm bg-gray-100 h-10 w-fit rounded-xl hover:bg-gray-200">
                        Car
                    </button>
                </div>
                ...
                <table class="min-w-full bg-white divide-y divide-gray-200 mb-20">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr v-for="(item) in items" :key="item.id" class="hover:bg-gray-100">
                      <!-- <td class="cursor-pointer px-4 py-3 whitespace-nowrap text-sm text-gray-900"><NuxtLink   :to="`/garage/${item.id}`">{{item.id}}</NuxtLink></td> -->
                      ...
                      <!-- <td class="cursor-pointer px-4 py-3 whitespace-nowrap text-sm text-gray-500"><NuxtLink   :to="`/garage/${item.id}`">{{item.priority}}</NuxtLink></td> -->
...
const { data, pending, error, refresh} = await useFetch(`${config.public.apiBase}/api/garage/ingarage2`, {
...
function onSearch() {
  ...
  router.replace({ query: { q: params.q, page: String(params.page) } })
    refresh()
}
