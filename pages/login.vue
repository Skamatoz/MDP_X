...
<template>
  <div class="login-page w-full h-full bg-white">
      ...
      <div class="w-full h-screen flex items-center justify-center">
          <div class="h-96 w-96 bg-black rounded-2xl p-6">
              <h1 class="w-full flex items-center justify-center text-xl mt-6 mb-6 text-white">Welcome to&nbsp;<span class="font-bold">MDP CRM</span></h1>
             <form action="" @submit.prevent="handleLogin">
                 <label for="Email" >
                     <span class="ml-2 text-sm font-medium text-white"> Email </span>
                     <input
                     v-model="email"
                       type="email"
                       id="email"
                   ...
                   <label for="Password">
                       <span class="ml-2 text-sm font-medium text-white"> Password </span>
                       <input
                       v-model="password"
                         type="password"
                         id="password"
                     ...
                     <button type="submit" class="flex items-center justify-center gap-3 mt-5 w-full h-12 rounded-xl  bg-orange-400 text-white font-medium cursor-pointer">
...
const handleLogin = async () => {
  ...
  try {
    ...
    if(data.user.role === 'STAFF' ||data.user.role === 'ADMIN'|| data.user.role === 'SUPERADMIN' ){
...
<!-- КОД ДЛЯ ТЕСТОВОГО СЕРВЕРА -->

<!-- <template>
  <div class="login-page w-full h-full bg-white">

      <div class="w-full h-screen flex items-center justify-center">
          <div class="h-96 w-96 bg-[#F7F7F8] rounded-2xl p-6 border-2 border-red-500">
              <h1 class="w-full flex items-center justify-center text-xl mt-6 mb-6 text-red-600">Welcome to&nbsp;<span class="font-bold">TEST VERSION</span></h1>
             <form action="" @submit.prevent="handleLogin">
                 <label for="Email" >
